...
for variant in "${variants[@]}"; do
	...
	jq -s \
		...
		--arg cuda_devices "$CUDA_DEVICES" \
		--arg rocr_devices "$ROCR_DEVICES" \
      ...
      | (if has("containerEnv") and (.containerEnv | has("ROCR_VISIBLE_DEVICES")) then
           .containerEnv.ROCR_VISIBLE_DEVICES = $rocr_devices
         else . end)
	  | (if has("containerEnv") and (.containerEnv | has("CUDA_VISIBLE_DEVICES")) then
           .containerEnv.CUDA_VISIBLE_DEVICES = $cuda_devices
         else . end)
      | (if $userns != "" then
	...
	for f in create_user.template.sh postStartCommand.template.sh; do
